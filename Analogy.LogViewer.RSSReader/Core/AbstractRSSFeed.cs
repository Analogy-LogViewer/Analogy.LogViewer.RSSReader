...
namespace Analogy.LogViewer.RSSReader.Core
{
    ...
    public abstract class AbstractRSSFeed : IRSSFeed
    {
        ...
        protected abstract Task<List<IRSSPost>> RefreshCurrentItems(bool suppressErrorDisplay);
        public virtual async Task<List<IRSSPost>> GetAllItemsFromWeb(bool onlyUnreadItems, bool suppressErrorDisplay, bool showHiddenPosts = false)
        {
            ...
            return items.OrderBy(x => x.Date ?? DateTime.MinValue).ThenByDescending(x => x.AddedDate).ToList();
        }
